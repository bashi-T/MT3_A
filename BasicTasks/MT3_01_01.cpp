...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Vector3 TriangleVertex1{0.0f, 1.0f, 0.0f};
	Vector3 TriangleVertex2{-1.0f, -1.0f, 0.0f};
	Vector3 TriangleVertex3{1.0f, -1.0f, 0.0f};
	...
	while (Novice::ProcessMessage() == 0) {
		...
		for (uint32_t i = 0; i < 3; ++i) {
			Vector3 ndcVertex = Transform(
				kLocalVertices[i], worldViewProjectionMatrix);
		...
		Novice::DrawTriangle(
		    ...
		    int(screenVertices[2].x), int(screenVertices[2].y), RED,
		    kFillModeSolid);
		    
		Novice::ScreenPrintf( 0, 50, "%f:%f:%f:%f:%f:%f", 
